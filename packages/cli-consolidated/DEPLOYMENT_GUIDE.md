# eve CLI - Complete Deployment Guide
...
```bash
eve deploy --domain monsite.com --provider opencode --website
```
...
- Commande: `eve dev` (ouvre OpenCode)
...
```bash
# Cloner le template
cd ~/.eve/deployments/monsite.com/website
...
# Modifier config
eve config set key value

# Voir credentials
eve config credentials
```
...
# Recréer certificats
rm -rf ~/.eve/deployments/monsite.com/traefik_certs
eve ignite
```
