 ...
 * Provides high-level operations for eve state synchronization
 */

import { APIClient, createAPIClient as createBaseClient } from '../domains/shared/lib/api-client.js';
import { logger } from '../utils/logger.js';
import type { eveConfig } from './config-service.js';
...
/**
 * API Service
 * Wraps the Synap backend API client with eve-specific operations
 */
export class APIService {
  ...
  async initialize(config?: Partial<APIServiceConfig>): Promise<void> {
    ...
    const baseUrl = config?.baseUrl || process.env.eve_POD_URL || process.env.SYNAP_POD_URL;
    const apiKey = config?.apiKey || process.env.eve_API_KEY || process.env.SYNAP_API_KEY;
    const workspaceId = config?.workspaceId || process.env.eve_WORKSPACE_ID || "default";
  ...
  /**
   * Push eve config to Synap backend
   */
  async pushToSynap(config: eveConfig): Promise<void> {
    if (!this.client) {
  ...
  async pullFromSynap(hearthId?: string): Promise<NormalState> {
    ...
    const id = hearthId || process.env.eve_HEARTH_ID;
    if (!id) {
      throw new APIServiceError(
    ...
    try {
      ...
      const state: NormalState = {
        config: {} as eveConfig, // Will be merged with local
        entities: {
          hearthNode: {
