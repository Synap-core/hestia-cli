...
export class APIClient {
  ...
  async createEntity(params: {
    ...
    const result = await this.request<{ result?: { data?: { id: string } }; error?: { message: string } }>(
  ...
  async updateEntity(
    ...
    const result = await this.request<{ result?: { data?: boolean }; error?: { message: string } }>(
   ...
   */
  async listEntities(profileSlug: string): Promise<Array<{ id: string; properties?: Record<string, unknown> }>> {
    const result = await this.request<{ result?: { data?: { entities: Array<{ id: string; properties?: Record<string, unknown> }> } }; error?: { message: string } }>(
   ...
   */
  async getEntity(entityId: string): Promise<{ id: string; properties?: Record<string, unknown> } | null> {
    const result = await this.request<{ result?: { data?: { id: string; properties?: Record<string, unknown> } }; error?: { message: string } }>(
  ...
  async getHearthStatus(hearthNodeId: string): Promise<{
    ...
    let provider: IntelligenceProvider | undefined;
    const providerId = hearth.properties?.intelligence_provider_id;
    if (providerId) {
