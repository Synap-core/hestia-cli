...
export class PackageService {
  ...
  private generateEnvVars(
    ...
    const env: Record<string, string> = {
      eve_PACKAGE_NAME: pkg.name,
      eve_PACKAGE_VERSION: pkg.version,
  ...
  private async startBinaryService(
    ...
    const serviceFile = `/etc/systemd/system/eve-${packageName}.service`;

    const serviceContent = `
[Unit]
Description=eve ${packageName}
...
`;
    ...
    await execAsync(`sudo systemctl enable eve-${packageName}`);
    await execAsync(`sudo systemctl start eve-${packageName}`);
  }

  private async stopBinaryService(packageName: string): Promise<void> {
    await execAsync(`sudo systemctl stop eve-${packageName}`);
  }
