// @ts-nocheck
/**
 * Unified State Manager for eve CLI
 *
 * Manages three layers of state with bidirectional sync:
 * - Normal State: Synap Backend entities + ~/.eve/config.yaml
...
import { loadConfig, saveConfig, getConfigPaths, eveConfig } from '../../../utils/index.js';
...
export class UnifiedStateManager {
   ...
   */
  async setNormalState(updates: Partial<eveConfig>): Promise<NormalState> {
    if (this.isSyncing) {
  ...
  /**
   * Translate eve state to OpenClaude profile format
   */
  translateToOpenClaude(eveState: NormalState): OpenClaudeProfile {
    const config = eveState.config;
    ...
    return {
      ...
      profile: {
        ...
        integrations: {
          synap: {
            enabled: true,
            podUrl: this.getEnvVar("eve_POD_URL") ?? this.getEnvVar("SYNAP_POD_URL"),
            apiKey: this.getEnvVar("eve_API_KEY") ?? this.getEnvVar("SYNAP_API_KEY"),
            workspaceId: eveState.entities.workspace?.id,
          },
          openclaw: {
  ...
  /**
   * Translate eve state to OpenClaw config format
   */
  translateToOpenClaw(eveState: NormalState): OpenClawConfig {
    const config = eveState.config;
    ...
    return {
      ...
      integrations: {
        synap: {
          ...
          apiKey: this.getEnvVar("eve_API_KEY") ?? this.getEnvVar("SYNAP_API_KEY"),
          podUrl: this.getEnvVar("eve_POD_URL") ?? "http://localhost:4000",
        },
      },
      lastSynced: new Date().toISOString(),
    };
  ...
  /**
   * Sync environment variables from eve state
   */
  syncEnvironment(eveState: NormalState): Record<string, string> {
    const config = eveState.config;
    const envUpdates: Record<string, string> = {};

    // Hearth configuration
    if (config.hearth.id) {
      envUpdates.eve_HEARTH_ID = config.hearth.id;
    }
    if (config.hearth.name) {
      envUpdates.eve_HEARTH_NAME = config.hearth.name;
    }
    if (config.hearth.role) {
      envUpdates.eve_HEARTH_ROLE = config.hearth.role;
    }
    if (config.hearth.domain) {
      envUpdates.eve_DOMAIN = config.hearth.domain;
    }

    // Intelligence configuration
    if (config.intelligence) {
      envUpdates.eve_INTELLIGENCE_PROVIDER = config.intelligence.provider;
      envUpdates.eve_INTELLIGENCE_MODEL = config.intelligence.model;

      if (config.intelligence.endpoint) {
        envUpdates.eve_INTELLIGENCE_ENDPOINT = config.intelligence.endpoint;
      }
      if (config.intelligence.apiKey) {
        envUpdates.eve_INTELLIGENCE_API_KEY = config.intelligence.apiKey;
      }
    }
  ...
  private async pushToSynap(localState: LocalState, result: SyncResult): Promise<void> {
    ...
    const updates: Partial<eveConfig> = {};
  ...
  private handleFileChange(filePath: string, eventType: string): void {
    ...
    // Invalidate cache for normal state if eve config changed
    if (filePath.includes(".eve")) {
      this.normalStateCache = null;
      this.normalStateCacheTime = null;
    }
