...
import { loadConfig, saveConfig, getConfigPaths, HestiaConfig } from '../../../utils/index.js';
...
import type { PackageInstance, HestiaError } from '../../lib/types/index';
  ...
  async restoreBackup(name: string, options: RecoveryOptions = {}): Promise<void> {
    ...
    try {
      ...
      if (backup.components.config) {
        ...
        const config = YAML.load(configContent) as HestiaConfig;
