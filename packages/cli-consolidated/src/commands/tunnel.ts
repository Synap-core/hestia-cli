...
import { pangolinService, PangolinService, type TunnelMode, type TunnelInfo, type PangolinStatus } from '../lib/domains/services/lib/pangolin-service.js';
import { logger, section } from '../lib/utils/index.js';
import { spinner, withSpinner } from '../lib/utils/index.js';
import { loadConfig, updateConfig } from '../lib/utils/index.js';
...
export function tunnelCommand(program: Command): void {
  ...
  tunnelCmd
    ...
    .action(async (options: TunnelOptions & { addTunnel?: string; removeTunnel?: string }) => {
 ...
 */
async function showTunnelStatus(options: TunnelOptions): Promise<void> {
  try {
 ...
 */
async function enableTunnel(options: TunnelOptions): Promise<void> {
  try {
    ...
    // Determine mode
    let mode: TunnelMode = options.mode || 'client';
 ...
 */
async function configureServerMode(options: TunnelOptions): Promise<void> {
  logger.newline();
 ...
 */
async function configureClientMode(options: TunnelOptions): Promise<void> {
  logger.newline();
  ...
  if (!serverUrl) {
    const answers = await inquirer.prompt<{ serverUrl: string }>([{
      type: 'input',
      name: 'serverUrl',
  ...
  const success = pangolinService.configureClient(serverUrl as string, token as string);
 ...
 * Disable tunnel
 */
async function disableTunnel(options: TunnelOptions): Promise<void> {
  try {
    const status = await pangolinService.getPangolinStatus();
 ...
 */
async function showDetailedStatus(options: TunnelOptions): Promise<void> {
  try {
    const status = await pangolinService.getPangolinStatus();
    const tunnelInfo = await pangolinService.getStatus();
 ...
 * Show tunnel URL
 */
async function showTunnelUrl(options: TunnelOptions): Promise<void> {
  try {
 ...
 * Show tunnel logs
 */
async function showTunnelLogs(options: TunnelOptions): Promise<void> {
  try {
 ...
 * List active tunnels
 */
async function listTunnels(options: TunnelOptions): Promise<void> {
  try {
    const tunnels = await pangolinService.listTunnels();
 ...
 */
async function updateTunnelConfig(options: TunnelOptions & { addTunnel?: string; removeTunnel?: string }): Promise<void> {
  try {
    ...
    if (addTunnel) {
      ...
      const newTunnels = [
        ...
        { name, localPort: port, protocol: 'tcp' as const },
      ];
...
function formatTunnelStatus(status: string): string {
  switch (status) {
    case 'connected':
