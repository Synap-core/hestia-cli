...
function formatStatus(status: ServiceStatus): string {
  const statusConfig: Record<ServiceStatus, { icon: string; color: string }> = {
...
export function servicesCommand(program: Command): void {
...
// List all services
async function listServices(options: { category?: string; json?: boolean; verbose?: boolean }): Promise<void> {
  try {
    ...
    const categories = options.category 
      ? [options.category as ServiceCategory]
...
async function listAvailableServices(options: { category?: string; json?: boolean }): Promise<void> {
  try {
    ...
    for (const service of availableServices) {
      ...
      if (service.dependencies.length > 0) {
        const deps = service.dependencies.map(d => 
          `${d.name}${d.optional ? ' (optional)' : ''}`
        ).join(', ');
...
// Install a service
async function installService(name: string, options: { port?: string; yes?: boolean }): Promise<void> {
  try {
...
// Remove a service
async function removeService(name: string, options: { force?: boolean }): Promise<void> {
  try {
    if (!isValidService(name)) {
      logger.error(`Unknown service: ${name}`);
      process.exit(1);
...
// Stop a service
async function stopService(name: string): Promise<void> {
  try {
    if (!isValidService(name)) {
      logger.error(`Unknown service: ${name}`);
      process.exit(1);
...
// Show status
async function showStatus(name: string | undefined, options: { json?: boolean }): Promise<void> {
  try {
    if (name) {
      if (!isValidService(name)) {
        logger.error(`Unknown service: ${name}`);
      ...
      if (status.version) console.log(`Version: ${chalk.cyan(status.version)}`);
      if (status.url) console.log(`URL: ${chalk.cyan(status.url)}`);
      if (status.uptime) console.log(`Uptime: ${chalk.cyan(formatUptime(status.uptime))}`);
      if (status.message) console.log(`Message: ${chalk.gray(status.message)}`);
      if (status.lastError) console.log(`Last Error: ${chalk.red(status.lastError)}`);
...
async function watchStatus(name: string | undefined, options: { json?: boolean }): Promise<void> {
  const interval = 2000;
...
// Disable a service
async function disableService(name: string): Promise<void> {
  try {
    if (!isValidService(name)) {
      logger.error(`Unknown service: ${name}`);
      process.exit(1);
...
// Configure a service
async function configureService(name: string, options: { port?: string[]; env?: string[]; file?: string }): Promise<void> {
  try {
    if (!isValidService(name)) {
      logger.error(`Unknown service: ${name}`);
      process.exit(1);
...
// Show logs
async function showLogs(name: string, options: { lines?: string; follow?: boolean }): Promise<void> {
  try {
    if (!isValidService(name)) {
      logger.error(`Unknown service: ${name}`);
      process.exit(1);
