...
import { createInitialConfig, configExists, updateConfig } from '../lib/utils/index.js';
import { checkPodHealth } from '../lib/domains/shared/lib/api-client.js';
import { getOrCreateeveWorkspace } from '../lib/domains/provision/lib/workspace-setup.js';
import type { IntelligenceConfig, OptionalServiceConfig } from '../lib/types/index.js';
import { serviceManager } from '../lib/domains/services/lib/service-manager.js';

export function initCommand(program: Command): void {
  program
    .command("init")
    .description("Initialize a new eve hearth")
    ...
    logger.info("🔥 Welcome to eve — The Digital Hearth\n");
      ...
      const { overwrite } = await inquirer.prompt([
        {
          type: "confirm",
          name: "overwrite",
          message: "eve is already initialized. Reinitialize?",
          default: false,
        },
      ]);
    ...
    if (options.quick) {
      ...
      if (aiPlatform === "opencode") {
        ...
        logger.info(`   • Configure: eve config set ai.platform.opencode.apiKey <your-key>`);
      } else if (aiPlatform === "openclaude") {
        logger.info(`\n🤖 OpenClaude setup:`);
        logger.info(`   • Get your API key at: https://openclaude.ai/settings/api-keys`);
        logger.info(`   • Configure: eve config set ai.platform.openclaude.apiKey <your-key>`);
      } else if (aiPlatform === "later") {
        logger.info(`\n⏳ AI Platform setup:`);
        logger.info(`   • Choose OpenCode or OpenClaude when ready`);
        logger.info(`   • Configure: eve config set ai.platform <opencode|openclaude>`);
      }
      
      logger.info(`\nNext steps:`);
      logger.info("  1. eve ignite          # Start the hearth");
      logger.info("  2. eve status          # Check status");
      logger.info("  3. eve add gateway     # Add OpenClaw gateway");
    ...
    let eveConfig = await createInitialConfig({
    ...
    if (config.optionalServices.length > 0) {
      ...
      eveConfig = await updateConfig({
        optionalServices: {
          ...eveConfig.optionalServices,
          ...optionalServicesConfig,
        },
      });
    }

    // Create eve workspace on the pod if API key is available
    if (health.healthy && !options.skipRegistration) {
      // Resolve API key: flag > env > prompt
      let apiKey: string = options.apiKey || process.env.eve_API_KEY || "";
      ...
      if (apiKey) {
        logger.info("\n🏗️  Setting up eve workspace on your data pod...");

        const wsResult = await getOrCreateeveWorkspace({
        ...
        if (wsResult.success) {
          ...
          logger.warn("   Run: eve connect --api-key <key>  to retry later.\n");
        }
      } else {
        logger.info("\nℹ️  Skipping workspace setup (no API key provided).");
        logger.info("   Run: eve connect --api-key <key>  when ready.\n");
      }
    }

    // Success!
    logger.info("\n✅ eve initialized successfully!\n");
    logger.info(`Hearth ID: ${eveConfig.hearth.id}`);
    logger.info(`Name: ${eveConfig.hearth.name}`);
    logger.info(`Role: ${eveConfig.hearth.role}`);
    if (eveConfig.pod?.workspaceId) {
      logger.info(`Workspace: ${eveConfig.pod.workspaceId}`);
      logger.info(`Pod: ${eveConfig.pod.url}`);
    }
    
    // AI Platform guidance
    if (eveConfig.aiPlatform === "opencode") {
      logger.info(`\n📝 OpenCode setup:`);
      logger.info(`   • Get your API key at: https://opencode.ai/api-keys`);
      logger.info(`   • Configure: eve config set ai.platform.opencode.apiKey <your-key>`);
    } else if (eveConfig.aiPlatform === "openclaude") {
      logger.info(`\n🤖 OpenClaude setup:`);
      logger.info(`   • Get your API key at: https://openclaude.ai/settings/api-keys`);
      logger.info(`   • Configure: eve config set ai.platform.openclaude.apiKey <your-key>`);
    } else if (eveConfig.aiPlatform === "later") {
      logger.info(`\n⏳ AI Platform setup:`);
      logger.info(`   • Choose OpenCode or OpenClaude when ready`);
      logger.info(`   • Configure: eve config set ai.platform <opencode|openclaude>`);
    }
    ...
    logger.info(`  2. eve status           # Check status`);
    logger.info(`  3. eve add gateway      # Add OpenClaw gateway`);
    logger.info(`  4. eve add builder      # Add OpenClaude builder`);
    logger.info(`\nFor help: eve --help`);
    logger.info(`Documentation: https://eve.sh/docs`);
  });
}
