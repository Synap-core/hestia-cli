...
import { getConfig } from '../lib/utils/index.js';
import { APIClient } from '../lib/domains/shared/lib/api-client.js';
import { logger, table } from '../lib/utils/index.js';
import { withSpinner } from '../lib/utils/index.js';
import { PackageService } from '../lib/domains/registry/lib/package-service.js';
...
export function packageCommand(program: Command): void {
    ...
    .action(async (packageName: string, options: { json?: boolean }) => {
      try {
        ...
        if (pkg.endpoints?.length) {
          ...
          pkg.endpoints.forEach((endpoint: string) => {
  ...
  pkgCmd
    ...
    .action(async (packageName: string, options: { follow?: boolean; lines?: string }) => {
      try {
        const config = await getConfig();
  ...
  pkgCmd
    ...
    .action(async (packageName: string | undefined, options: { all?: boolean; force?: boolean }) => {
      try {
        const config = await getConfig();
        const api = new APIClient(config);
        const pkgService = new PackageService(config);
  ...
  pkgCmd
    ...
    .allowUnknownOption()
    .action(async (packageName: string, command: string, _options: any, commandObj: Command) => {
      try {
        const config = await getConfig();
        const pkgService = new PackageService(config);
...
function formatHealth(health?: string): string {
  if (!health) return chalk.gray('unknown');
  const colors: Record<string, (s: string) => string> = {
