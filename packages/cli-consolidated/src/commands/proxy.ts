...
// Get eve home directory
function geteveHome(): string {
  return process.env.eve_HOME || '/opt/eve';
}

// Detect current reverse proxy
function detectCurrentProxy(): ProxyType | null {
  // Check environment variable
  const envProxy = process.env.eve_REVERSE_PROXY;
  ...
  try {
    ...
    if (stdout.includes('eve-traefik')) return 'traefik';
    if (stdout.includes('eve-nginx')) return 'nginx';
  } catch {
    // Ignore errors
  }
  ...
  // Check config file
  try {
    const envFile = path.join(geteveHome(), 'config', '.env');
    const content = fs.readFileSync(envFile, 'utf-8');
    const match = content.match(/eve_REVERSE_PROXY=(\w+)/);
    if (match) {
      const proxy = match[1];
...
async function getProxyStatus(): Promise<ProxyStatus> {
  ...
  const containerName = type === 'traefik' ? 'eve-traefik' : 'eve-nginx';
...
async function showProxyStatus(options: ProxyOptions): Promise<void> {
  ...
  if (!status.type) {
    ...
    logger.info('Run "eve proxy switch <nginx|traefik>" to configure a proxy');
    return;
  }
  ...
  if (status.dashboardUrl) {
    ...
    logger.info(`Run "eve proxy dashboard" to open`);
  }
...
async function switchProxy(newType: ProxyType): Promise<void> {
  ...
  await withSpinner(
    ...
    async () => {
      ...
      const envFile = path.join(eveHome, 'config', '.env');
      try {
        let envContent = await fs.readFile(envFile, 'utf-8').catch(() => '');
        
        // Remove old proxy setting
        envContent = envContent.replace(/^eve_REVERSE_PROXY=.*$/gm, '');
        
        // Add new setting
        envContent += `\neve_REVERSE_PROXY=${newType}\n`;
      ...
      const installScript = path.join(eveHome, 'install', 'src', 'phases', 'phase2.sh');
      if (await fileExists(installScript)) {
        await execa('sudo', ['eve_REVERSE_PROXY=' + newType, installScript], {
          cwd: eveHome,
          reject: false,
        });
      } else {
  ...
  if (newType === 'traefik') {
    ...
    logger.info(`Run "eve proxy dashboard" to open it`);
  }
}
...
async function openDashboard(): Promise<void> {
  ...
  if (status.type !== 'traefik') {
    ...
    logger.info('Run "eve proxy switch traefik" to use Traefik');
    process.exit(1);
  }
...
async function configureProxy(): Promise<void> {
  ...
  logger.info(`  ${chalk.cyan('eve proxy switch nginx')}   - Use Nginx (traditional)`);
  logger.info(`  ${chalk.cyan('eve proxy switch traefik')}  - Use Traefik (modern, dynamic)`);
  logger.info(`  ${chalk.cyan('eve proxy restart')}          - Restart current proxy`);
}
