 ...
 * Usage: eve os [subcommand]
 *
 * Manages the operating system for eve nodes including:
...
import { osManager, OSManager } from '../lib/domains/install/lib/os-manager.js';
...
export function osCommand(program: Command): void {
  ...
  osCmd
    ...
    .option('-p, --path <path>', 'Backup destination path', '/var/backups/eve')
    ...
    .action(async (options: { backup?: string; interactive?: boolean }) => {
      try {
        ...
        if (options.interactive || !backupPath) {
          ...
          const backupDir = '/var/backups/eve';
