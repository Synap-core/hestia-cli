...
import { WhoDBService } from '../lib/domains/services/lib/whodb-service.js';
...
export function dbViewerCommand(program: Command): void {
  ...
  dbViewer
    ...
    .action(async (cmdOptions: { port: string; ai: boolean }) => {
      try {
        const hestiaHome = process.env.HESTIA_HOME || "/opt/hestia";
        const service = new WhoDBService(hestiaHome);
        await service.initialize();
  ...
  dbViewer
    ...
    .action(async (options: DBViewerOptions) => {
      try {
        const hestiaHome = process.env.HESTIA_HOME || "/opt/hestia";
        const service = new WhoDBService(hestiaHome);
        await service.initialize();
        ...
        const dbViewerConfig = (config as any).dbViewer || {};
        const status = await service.getStatus();
        const url = service.getUrl();
  ...
  dbViewer
    ...
    .action(async (options: { enable: boolean; disable: boolean; model: string }) => {
      try {
        const hestiaHome = process.env.HESTIA_HOME || "/opt/hestia";
        const service = new WhoDBService(hestiaHome);
        await service.initialize();
