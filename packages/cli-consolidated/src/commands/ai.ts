#!/usr/bin/env node
/**
 * ai command - OpenClaude integration for eve
 *
...
export function aiCommand(program: Command): void {
  ...
  program
    ...
    .action(async (options: { setup?: boolean }) => {
      try {
        logger.header('eve AI - OPENCLAUDE');
        ...
        // Sync eve config to OpenClaude
        logger.info('Syncing eve configuration to OpenClaude...');
        await synceveToOpenClaude();
    ...
    .action(async (options: AIStatusOptions) => {
      try {
        ...
        if (providerConfig) {
          ...
          logger.warn('No provider configured. Run `eve ai:configure` to set up.');
        }
        ...
        if (mcpServers.length === 0) {
          ...
          logger.info(`Use ${chalk.cyan('eve ai:mcp add')} to install servers.`);
        } else {
  ...
  program
    ...
    .action(async (options: {
      ...
      try {
        ...
        // Save to eve config via state manager
        logger.info('Syncing configuration to eve...');
        await syncConfigToeve(config);
      ...
      try {
        ...
        switch (action) {
          case 'list': {
            ...
            if (servers.length === 0) {
              logger.info('No MCP servers installed.');
              logger.info(`Use ${chalk.cyan('eve ai:mcp add')} to install a server.`);
            } else {
...
async function runSetup(options: AISetupOptions): Promise<void> {
  try {
    ...
    // eve MCP
    try {
      spinner.start('mcp-hearth', 'Installing eve MCP server...');
      await openclaudeService.installMCPServer('eve', {
        name: 'eve',
        command: 'npx',
        args: ['-y', '@synap/mcp-hearth', 'start'],
        env: {},
      });
      spinner.succeed('mcp-hearth', 'eve MCP server installed');
    } catch (error: any) {
      spinner.fail('mcp-hearth', `Failed to install eve MCP: ${error.message}`);
    }
...
async function synceveToOpenClaude(): Promise<void> {
  await stateManager.syncAll();
}

async function syncConfigToeve(config: {
  provider: string;
  model: string;
  apiKey?: string;
  endpoint?: string;
