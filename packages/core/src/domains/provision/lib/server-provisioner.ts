...
export class ServerProvisioner {
    ...
    const defaultProfiles: ProvisionProfile[] = [
      {
        ...
        network: { dhcp: true },
        packages: ['eve-core', 'docker'],
        optimizations: ['basic'],
      },
      {
        name: 'standard',
        type: 'standard',
        description: 'Standard eve deployment',
        minCpu: 4,
        ...
        network: { dhcp: true },
        packages: ['eve-core', 'docker', 'synap-stack', 'monitoring'],
      ...
      },
      {
        ...
        network: { dhcp: true },
        packages: ['eve-core', 'docker', 'synap-stack', 'ollama', 'cuda', 'monitoring'],
  ...
  async generateDocumentation(report: ProvisionReport): Promise<string> {
    const doc = [
      `# eve Server Provisioning Report`,
      ``,
  ...
  async detectOtherNodes(): Promise<Array<{ hostname: string; ip: string; status: string }>> {
    this.log('info', 'Detecting other eve nodes on network...');

    const nodes: Array<{ hostname: string; ip: string; status: string }> = [];

    try {
      const { stdout } = await execAsync('avahi-browse -r -t _eve._tcp 2>/dev/null || echo ""');
      const lines = stdout.split('\n');
