 ...
 * Usage: eve recovery:<subcommand>
 */

import { Command } from 'commander';
import { recovery, RecoverySystem, BackupMetadata, DiagnosisResult, SuggestedFix, RecoveryOptions } from '../../../domains/services/lib/recovery.js';
...
export function recoveryCommand(program: Command): void {
  ...
  program
    ...
    .action(async (options: BackupOptions) => {
      try {
        ...
        logger.info(`Location: ${chalk.cyan('~/.eve/backups/')}`);
    ...
    .action(async () => {
      try {
        ...
        if (backups.length === 0) {
          ...
          logger.info(`Create a backup with: ${chalk.cyan('eve recovery:backup')}`);
          return;
        }
  ...
  program
    ...
    .action(async (options: RestoreOptions) => {
      try {
        let backupName = options.name;
        let backupToRestore: BackupMetadata | undefined;
        ...
        logger.newline();
        logger.info(chalk.yellow('Note: You may need to run `eve ignite` to fully restart all packages.'));
      } catch (error: any) {
  ...
  program
    ...
    .action(async () => {
      try {
        ...
        logger.info('Run: ' + chalk.cyan('eve recovery:safe-mode'));
        logger.info('Or:  ' + chalk.cyan('eve recovery:rollback --installation'));
      } catch (error: any) {
    ...
    .action(async (options: DiagnoseOptions) => {
      try {
        ...
        if (diagnosis.suggestions.length > 0) {
          ...
          logger.newline();
          logger.info(`Apply fixes with: ${chalk.cyan('eve recovery:auto')}`);
        }
    ...
    .action(async (options: AutoOptions) => {
      try {
        ...
        if (diagnosis.suggestions.length === 0) {
          ...
          logger.info(`Run ${chalk.cyan('eve recovery:diagnose')} for more details.`);
          return;
        }
...
async function generateDiagnosisReport(diagnosis: DiagnosisResult): Promise<string> {
  const report = {
  ...
  const reportDir = path.join(os.homedir(), '.eve', 'reports');
