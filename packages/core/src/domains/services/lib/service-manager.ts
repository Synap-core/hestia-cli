 ...
 * Service Manager
 * 
 * Central service management for eve.
...
} from '../../../domains/services/lib/optional-services.js';
...
class ServiceManager {
  ...
  async initialize(): Promise<void> {
    ...
    const target = process.env.eve_TARGET || '/opt/eve';
  ...
  async enable(serviceName: string): Promise<void> {
    ...
    const state = this.states.get(serviceName);
    if (!state?.installed) {
      throw new Error(`${service.displayName} is not installed. Run 'eve services:install ${serviceName}' first.`);
    }
  ...
  async start(serviceName: string): Promise<void> {
    ...
    if (!state.enabled) {
      throw new Error(`${service.displayName} is not enabled. Run 'eve services:enable ${serviceName}' first.`);
    }
