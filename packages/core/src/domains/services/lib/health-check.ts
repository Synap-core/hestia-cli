// @ts-nocheck
/**
 * Health Check System for eve CLI
 *
 * Comprehensive real-time monitoring system for all eve services.
...
export class HealthCheckSystem extends EventEmitter {
  ...
  /**
   * Check disk space on /opt/eve
   */
  async checkDiskSpace(): Promise<HealthCheckResult> {
    const startTime = Date.now();
    try {
      const { stdout } = await execAsync(
        'df -h /opt/eve 2>/dev/null || df -h / 2>/dev/null || echo "Filesystem Size Used Avail Use%"',
        { timeout: 5000 }
      );
  ...
  async checkBackupStatus(): Promise<HealthCheckResult> {
    ...
    try {
      ...
      const backupDir = '/opt/eve/backups';
  ...
  private async restartService(serviceName: string): Promise<void> {
    const commands: Record<string, string> = {
      synapBackend: 'cd /opt/eve/synap-backend && docker-compose restart backend',
