...
export function installCommand(program: Command): void {
  program
    ...
    .description('Run eve installation (phase1, phase2, phase3, or all)')
    .argument('[phase]', 'Installation phase', 'all')
    .option('-t, --target <path>', 'Installation target directory', '/opt/eve')
    ...
    .action(async (phase: InstallPhase | 'all', options: InstallOptions) => {
      try {
        ...
        logger.info(`Target directory: ${chalk.cyan(options.target || '/opt/eve')}`);
        logger.newline();
        ...
        if (phase === 'all' || phase === 'phase3') {
          logger.newline();
          section('Next Steps');
          logger.info(`1. Initialize eve: ${chalk.cyan('eve init')}`);
          logger.info(`2. Add packages: ${chalk.cyan('eve add <package>')}`);
          logger.info(`3. Ignite hearth: ${chalk.cyan('eve ignite')}`);
        }
...
async function runPreflightChecks(options: InstallOptions): Promise<boolean> {
  ...
  const checks = [
    ...
    { name: 'Target directory writable', fn: () => checkDirectory(options.target || '/opt/eve') },
...
async function runPhase(phase: InstallPhase, options: InstallOptions): Promise<void> {
  ...
  try {
    ...
    const altPaths = [
      `/opt/eve/install/${phase}.sh`,
      `./packages/install/src/phases/${phase}.sh`,
      `${process.env.HOME}/.eve/install/${phase}.sh`,
    ];
  ...
  const env = {
    ...process.env,
    eve_TARGET: options.target || '/opt/eve',
    eve_SAFE_MODE: options.safeMode ? '1' : '0',
    eve_UNATTENDED: options.unattended ? '1' : '0',
  };
...
function getPhaseScript(phase: InstallPhase): string {
  if (phase === 'all') {
    return '/opt/eve/install/phases/phase1.sh';
  }
  const scriptMap: Record<Exclude<InstallPhase, 'all'>, string> = {
    phase1: '/opt/eve/install/phases/phase1.sh',
    phase2: '/opt/eve/install/phases/phase2.sh',
    phase3: '/opt/eve/install/phases/phase3.sh',
  };
