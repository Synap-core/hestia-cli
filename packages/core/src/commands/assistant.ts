...
export function assistantCommand(program: Command): void {
  ...
  assistant
    ...
    .action(async (options: AssistantOptions & { port: string; apiPort: string }) => {
      try {
  ...
  assistant
    ...
    .action(async (platform: string) => {
      try {
        await configureComms(platform.toLowerCase() as CommsPlatform);
      } catch (error: any) {
  ...
  assistant
    ...
    .action(async (message: string, options: { platform?: string }) => {
      try {
 ...
 */
async function startAssistant(options: AssistantOptions & { port: string; apiPort: string }): Promise<void> {
...
async function showStatus(): Promise<void> {
  ...
  const platforms: CommsPlatform[] = ['telegram', 'whatsapp', 'discord', 'imessage'];
 ...
 */
async function setupAssistant(): Promise<void> {
 ...
 * Manage skills
 */
async function manageSkills(action: string): Promise<void> {
...
async function toggleSkill(enable: boolean): Promise<void> {
  const skills = await openclawService.listSkills();
 ...
 */
async function configureComms(platform: CommsPlatform): Promise<void> {
  logger.header(`CONFIGURE ${platform.toUpperCase()}`);

  const validPlatforms: CommsPlatform[] = ['telegram', 'whatsapp', 'discord', 'imessage'];
...
async function configureCommsPlatformInteractive(platform: CommsPlatform): Promise<void> {
  const config: { enabled: boolean; botToken?: string; apiKey?: string; autoReply: boolean } = {
    enabled: true,
    autoReply: true,
  };
 ...
 */
async function sendMessage(message: string, platform?: string): Promise<void> {
  ...
  try {
    const response = await openclawService.sendMessage(message, {
      platform: platform as CommsPlatform,
    });
 ...
 */
async function createDefaultSkills(): Promise<void> {
  const defaults = [
    {
      name: 'help',
 ...
 */
function getSkillTemplate(language: string): string {
