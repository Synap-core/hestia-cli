...
import { HESTIA_DEFINITION } from "./hestia-definition.js";
 ...
 */
export async function createHestiaWorkspace(
 ...
 */
export async function findExistingHestiaWorkspace(
  podUrl: string,
  apiKey: string
): Promise<string | null> {
  try {
    ...
    const hestiaWorkspace = workspaces.find(
      (w: { name?: string; description?: string }) =>
        w.name?.toLowerCase().includes("hestia") ||
 ...
 */
export async function getOrCreateHestiaWorkspace(
  options: CreateWorkspaceOptions
): Promise<CreateWorkspaceResult> {
