...
import { loadConfig, getConfigPaths, updateConfig, saveConfig } from "./config.js";
import type { HestiaConfig } from "../types.js";
...
// WhoDB service configuration interface
...
export class WhoDBService {
   ...
   */
  async connectToSynap(): Promise<void> {
    ...
    let synapConfig: Record<string, string> = {};
    ...
    const whodbEnv: Record<string, string> = {
   ...
   */
  async enableAI(model?: string): Promise<void> {
   ...
   */
  async connectToDatabase(databaseName: string): Promise<void> {
   ...
   */
  private getWhoDBConfig(): WhoDBConfig {
    const dbViewer = (this.config as any).dbViewer;
    return {
   ...
   */
  private async updateConfig(updates: Partial<WhoDBConfig>): Promise<void> {
    ...
    await saveConfig(newConfig as HestiaConfig, this.configPath);
    this.config = newConfig as HestiaConfig;
  }

  /**
   * Add a database to the configured databases list
   */
  private async addDatabase(databaseName: string): Promise<void> {
    const current = this.getWhoDBConfig();
   ...
   */
  private generateEnvFile(config: WhoDBConfig): string {
   ...
   */
  private generateEnvFileContent(env: Record<string, string>): string {
    const lines = [
   ...
   */
  private generateBasicCompose(config: WhoDBConfig): string {
    return `services:
  whodb:
