...
export class HestiaTestSuite {
  ...
  private async testConfig(): Promise<TestResult> {
    ...
    try {
      ...
      const customConfig: HestiaConfig = {
        ...config,
        hearth: {
          ...config.hearth,
          name: "Test Hearth",
...
`,
  ...
  private async testPersistence(): Promise<TestResult> {
    ...
    try {
      ...
      const testConfig: Partial<HestiaConfig> = {
        hearth: {
          id: `test-${Date.now()}`,
      ...
      await saveConfig({ ...defaultConfig, ...testConfig } as HestiaConfig, this.testEnv!.configPath);
