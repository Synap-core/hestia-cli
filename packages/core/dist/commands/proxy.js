...
function detectCurrentProxy() {
    ...
    try {
        ...
        if (stdout.includes('hestia-traefik'))
            return 'traefik';
        if (stdout.includes('hestia-nginx'))
            return 'nginx';
    }
    catch {
        // Ignore errors
...
async function getProxyStatus() {
    ...
    try {
        const { stdout } = await execa('docker', ['ps', '--filter', `name=${containerName}`, '--format', '{{.Status}}|{{.Image}}'], { reject: false });
        if (stdout) {
...
async function showProxyStatus(options) {
    const status = await withSpinner('Checking proxy status...', () => getProxyStatus(), 'Status checked');
    if (options.json) {
...
async function switchProxy(newType) {
    ...
    await withSpinner(`Configuring ${newType}...`, async () => {
...
async function restartProxy() {
    ...
    await withSpinner(`Restarting ${status.type}...`, async () => {
...
// Main command export
export function proxyCommand(program) {
