...
function displayReport(report, verbose) {
    ...
    const categories = ["unit", "integration", "e2e", "smoke"];
 ...
 */
async function runTestsWithProgress(suite, options, category) {
...
export function testCommand(program) {
    ...
    program
        ...
        try {
            ...
            const report = await runTestsWithProgress(suite, config, category);
    ...
    program
        ...
        try {
            ...
            const suite = new HestiaTestSuite(config);
            const report = await runTestsWithProgress(suite, config, "unit");
            displayReport(report, options.verbose ?? false);
            process.exit(report.summary.failed > 0 ? 1 : 0);
        }
        catch (error) {
    ...
    program
        ...
        try {
            ...
            const suite = new HestiaTestSuite(config);
            const report = await runTestsWithProgress(suite, config, "integration");
            displayReport(report, options.verbose ?? false);
            process.exit(report.summary.failed > 0 ? 1 : 0);
        }
        catch (error) {
    ...
    program
        ...
        try {
            ...
            const suite = new HestiaTestSuite(config);
            const report = await runTestsWithProgress(suite, config, "e2e");
            displayReport(report, options.verbose ?? false);
            process.exit(report.summary.failed > 0 ? 1 : 0);
        }
        catch (error) {
    ...
    program
        ...
        try {
            ...
            const report = await runTestsWithProgress(suite, config, "smoke");
            displayReport(report, options.verbose ?? false);
            process.exit(report.summary.failed > 0 ? 1 : 0);
        }
        catch (error) {
    ...
    program
        ...
        try {
            ...
            let report;
            if (options.category) {
                report = await suite.runTests(options.category);
            }
            else {
                report = await suite.runAllTests();
            }
