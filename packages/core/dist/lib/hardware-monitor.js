...
export class HardwareMonitor {
    ...
    async cpuTemperature() {
        if (this._isLinux) {
            try {
                ...
                const cpuZones = zones.filter(z => z.type?.toLowerCase().includes('cpu') ||
            ...
            try {
                ...
                for (const line of lines) {
                    ...
                    if (line.includes('Pages free')) {
                        ...
                        if (match)
                            available = parseInt(match[1]) * pageSize;
                    }
                }
            }
            catch {
            ...
            try {
                ...
                for (const line of lines) {
                    ...
                    if (parts.length >= 14) {
                        ...
                        if (device.startsWith('loop') || /^\D+\d+\w*$/.test(device))
                            continue;
    ...
    async networkInterfaces() {
        ...
        for (const [name, addrs] of Object.entries(ifaces)) {
            if (!addrs)
                continue;
            const type = name.startsWith('lo') ? 'loopback' :
    ...
    async collectAll() {
        ...
        const [cpuInfo, cpuUsage, cpuLoad, cpuTemp, cpuFreq, memUsage, memInfo, swapUsage, memPressure, diskMounts, diskIO, diskHealth, diskInfo, netInterfaces, netUsage, netSpeed, netLatency, gpuInfo, gpuUsage, power, thermal, fans,] = await Promise.all([
            this.cpuInfo(),
            this.cpuUsage(),
            this.cpuLoad(),
    ...
    generateMarkdownReport(metrics) {
        ...
        report.push(`\n## Memory`, `- **Total:** ${metrics.memory.usage.total} GB`, `- **Used:** ${metrics.memory.usage.used} GB (${metrics.memory.usage.percentage}%)`, `- **Available:** ${metrics.memory.usage.available} GB`, `- **Swap:** ${metrics.memory.swap.used} GB / ${metrics.memory.swap.total} GB`, `- **Pressure:** ${metrics.memory.pressure.level} (${metrics.memory.pressure.details})`);
        report.push(`\n## Disk`);
        ...
        if (metrics.alerts.length > 0) {
            report.push(`\n## Alerts`, ...metrics.alerts.map(a => `- **[${a.severity.toUpperCase()}]** ${a.message}`));
        }
        return report.join('\n');
    }
