...
export class OSManager {
                ...
                case 'debian':
                    ...
                    for (const line of aptShow.split('\n')) {
                        if (line.startsWith('Package: '))
                            info.name = line.substring(9);
                        if (line.startsWith('Version: '))
                            info.version = line.substring(9);
                        if (line.startsWith('Description: '))
                            info.description = line.substring(13);
                        if (line.startsWith('Homepage: '))
                            info.homepage = line.substring(10);
                        if (line.startsWith('Maintainer: '))
                            info.maintainer = line.substring(12);
                        if (line.startsWith('Depends: '))
                            info.dependencies = line.substring(9).split(', ');
                        if (line.startsWith('Size: '))
                            info.size = this.formatBytes(parseInt(line.substring(6), 10));
                    }
                    // Check if installed
                    try {
                ...
                for (const line of output.split('\n')) {
                    ...
                    if (line.includes('Active:')) {
                        if (line.includes('active (running)'))
                            ...
                            info.status = 'failed';
                        else if (line.includes('activating'))
                            info.status = 'activating';
                        else if (line.includes('deactivating'))
                            info.status = 'deactivating';
                    }
                    if (line.includes('Main PID:')) {
                        const match = line.match(/Main PID:\s*(\d+)/);
                        if (match)
                            info.pid = parseInt(match[1], 10);
                    }
                }
                return info;
            }
    ...
    createUser(username, options = {}) {
        try {
            ...
            if (options.createHome !== false && !options.system)
            ...
            if (this.commandExists('smartctl')) {
                try {
                    ...
                    if (smartOutput.includes('Serial Number')) {
                        ...
                        if (serialMatch)
                            disk.serial = serialMatch[1].trim();
                    }
                }
                catch {
    ...
    formatDisk(device, filesystem = 'ext4') {
        try {
            ...
            // Safety check: ensure it'a real disk
...
`;
    ...
    getInitSystem() {
        if (this.cachedInitSystem)
            return this.cachedInitSystem;
        try {
            if (this.getPlatform() === 'darwin') {
    ...
    getInterfaceType(name) {
        ...
        if (name.startsWith('br'))
            return 'bridge';
        if (name.startsWith('vlan') || name.includes('.'))
            return 'vlan';
        return 'unknown';
    }
    getDNSServers() {
...
//# sourceMappingURL=os-manager.js.map