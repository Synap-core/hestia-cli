...
export function osCommand(program) {
    ...
    packagesCmd
        ...
        .action(async (options) => {
        try {
            const packages = await withSpinner('Fetching installed packages...', () => osManager.listInstalled(), `Found ${osManager.listInstalled().length} packages`);
            if (options.json) {
    ...
    packagesCmd
        ...
        try {
            ...
            const success = await withSpinner('Updating package lists...', () => osManager.updatePackages(), 'Package lists updated');
            if (!success) {
                process.exit(1);
            }
        }
    ...
    packagesCmd
        ...
        try {
            ...
            const success = await withSpinner('Upgrading packages...', () => osManager.upgradePackages(), 'Packages upgraded');
            if (!success) {
                process.exit(1);
            }
        }
    ...
    packagesCmd
        ...
        try {
            const success = await withSpinner(`Installing ${packageName}...`, () => osManager.installPackage(packageName), `Package ${packageName} installed`);
            if (!success) {
                process.exit(1);
            }
        }
    ...
    packagesCmd
        ...
        try {
            ...
            const success = await withSpinner(`Removing ${packageName}...`, () => osManager.removePackage(packageName), `Package ${packageName} removed`);
            if (!success) {
                process.exit(1);
            }
        }
    ...
    packagesCmd
        ...
        try {
            const packages = await withSpinner(`Searching for "${query}"...`, () => osManager.searchPackage(query), `Found packages matching "${query}"`);
            if (options.json) {
                console.log(JSON.stringify(packages, null, 2));
                return;
            }
    ...
    servicesCmd
        ...
        try {
            const success = await withSpinner(`Starting ${serviceName}...`, () => osManager.startService(serviceName), `Service ${serviceName} started`);
            if (!success) {
                process.exit(1);
            }
        }
    ...
    servicesCmd
        ...
        .action(async (serviceName) => {
        try {
            const success = await withSpinner(`Stopping ${serviceName}...`, () => osManager.stopService(serviceName), `Service ${serviceName} stopped`);
            if (!success) {
                process.exit(1);
            }
        }
    ...
    servicesCmd
        ...
        .action(async (serviceName) => {
        try {
            const success = await withSpinner(`Restarting ${serviceName}...`, () => osManager.restartService(serviceName), `Service ${serviceName} restarted`);
            if (!success) {
                process.exit(1);
            }
        }
    ...
    servicesCmd
        ...
        .action(async (serviceName) => {
        try {
            const success = await withSpinner(`Enabling ${serviceName}...`, () => osManager.enableService(serviceName), `Service ${serviceName} enabled`);
            if (!success) {
                process.exit(1);
            }
        }
    ...
    servicesCmd
        ...
        .action(async (serviceName) => {
        try {
            const success = await withSpinner(`Disabling ${serviceName}...`, () => osManager.disableService(serviceName), `Service ${serviceName} disabled`);
            if (!success) {
                process.exit(1);
            }
        }
    ...
    usersCmd
        ...
        try {
            ...
            const success = await withSpinner(`Creating user ${username}...`, () => osManager.createUser(username, userOptions), `User ${username} created`);
            if (!success) {
                process.exit(1);
            }
        }
    ...
    usersCmd
        ...
        try {
            ...
            const success = await withSpinner(`Deleting user ${username}...`, () => osManager.deleteUser(username, options.removeHome), `User ${username} deleted`);
            if (!success) {
                process.exit(1);
            }
        }
    ...
    usersCmd
        ...
        try {
            const success = await withSpinner(`Adding ${username} to group ${group}...`, () => osManager.addToGroup(username, group), `User ${username} added to group ${group}`);
            if (!success) {
                process.exit(1);
            }
        }
    ...
    usersCmd
        ...
        try {
            ...
            const success = await withSpinner(`Setting password for ${username}...`, () => osManager.setPassword(username, password), `Password set for user ${username}`);
            if (!success) {
                process.exit(1);
            }
        }
    ...
    networkCmd
        ...
        .action(async (name) => {
        try {
            const success = await withSpinner(`Setting hostname to ${name}...`, () => osManager.setHostname(name), `Hostname set to ${name}`);
            if (!success) {
                process.exit(1);
            }
        }
    ...
    networkCmd
        ...
        try {
            const success = await withSpinner(`Configuring DNS servers...`, () => osManager.configureDNS(servers), `DNS servers configured: ${servers.join(', ')}`);
            if (!success) {
                process.exit(1);
            }
        }
    ...
    firewallCmd
        ...
        .action(async () => {
        try {
            const success = await withSpinner('Enabling firewall...', () => osManager.enableFirewall(), 'Firewall enabled');
            if (!success) {
                process.exit(1);
            }
        }
    ...
    firewallCmd
        ...
        try {
            ...
            const success = await withSpinner('Disabling firewall...', () => osManager.disableFirewall(), 'Firewall disabled');
            if (!success) {
                process.exit(1);
            }
        }
    ...
    firewallCmd
        ...
        try {
            ...
            const success = osManager.allowPort(portNum, options.protocol, options.from);
            if (success) {
    ...
    diskCmd
        ...
        try {
            ...
            const success = await withSpinner(`Formatting ${device} with ${options.filesystem}...`, () => osManager.formatDisk(device, options.filesystem), `Disk ${device} formatted`);
            if (!success) {
                process.exit(1);
            }
        }
    ...
    diskCmd
        ...
        .action(async (device, path, options) => {
        try {
            const mountOpts = options.options?.split(',').map(o => o.trim()).filter(Boolean) || [];
            const success = await withSpinner(`Mounting ${device} to ${path}...`, () => osManager.mount(device, path, mountOpts), `Mounted ${device} to ${path}`);
            if (!success) {
                process.exit(1);
            }
        }
    ...
    diskCmd
        ...
        .action(async (path) => {
        try {
            const success = await withSpinner(`Unmounting ${path}...`, () => osManager.unmount(path), `Unmounted ${path}`);
            if (!success) {
                process.exit(1);
            }
        }
    ...
    sysctlCmd
        ...
        try {
            const success = await withSpinner(`Setting ${key} = ${value}...`, () => osManager.setSysctl(key, value, options.persistent !== false), `Parameter ${key} set to ${value}`);
            if (!success) {
                process.exit(1);
            }
        }
    ...
    osCmd
        ...
        .action(async (options) => {
        try {
            const report = await withSpinner('Generating OS report...', () => osManager.generateReport(), 'Report generated');
    ...
    osCmd
        ...
        try {
            ...
            const success = await withSpinner('Creating OS configuration backup...', () => osManager.backupConfig(options.path), 'Backup complete');
            if (!success) {
                process.exit(1);
            }
        }
    ...
    osCmd
        ...
        try {
            ...
            const success = await withSpinner('Restoring configuration...', () => osManager.restoreConfig(backupPath), 'Restore complete');
            if (!success) {
                process.exit(1);
            }
