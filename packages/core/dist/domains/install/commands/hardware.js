...
export function hardwareCommand(program) {
...
async function showCPUDetails(options) {
    const [cpuInfo, cpuUsage, cpuTemp, cpuFreq, cpuLoad,] = await Promise.all([
...
async function showMemoryDetails(options) {
    const [memUsage, memInfo, swapUsage, memPressure,] = await Promise.all([
    ...
    if (memInfo) {
        ...
        if (memInfo.type)
            logger.info(`Type: ${chalk.cyan(memInfo.type)}`);
        if (memInfo.speed)
            logger.info(`Speed: ${chalk.cyan(memInfo.speed + ' MHz')}`);
        if (memInfo.manufacturer)
            logger.info(`Manufacturer: ${chalk.cyan(memInfo.manufacturer)}`);
        if (memInfo.slots) {
            logger.info(`Slots: ${chalk.cyan(memInfo.slotsUsed + '/' + memInfo.slots + ' used')}`);
        }
        if (memInfo.voltage)
            logger.info(`Voltage: ${chalk.cyan(memInfo.voltage + 'V')}`);
    }
}
// ============================================================================
// DISK DETAILS
...
async function showDiskDetails(options) {
    const [diskMounts, diskIO, diskHealth, diskInfo,] = await Promise.all([
...
async function showNetworkDetails(options) {
    const [interfaces, stats, speed, latency,] = await Promise.all([
...
async function showGPUDetails(options) {
    const [gpuInfo, gpuUsage,] = await Promise.all([
    ...
    for (const info of gpuInfo) {
        ...
        if (info.driver)
            logger.info(`Driver: ${chalk.cyan(info.driver)}`);
        if (info.pci)
            logger.info(`PCI: ${chalk.cyan(info.pci)}`);
        logger.newline();
        if (usage) {
...
async function showThermalDetails(options) {
    const [thermal, fans,] = await Promise.all([
