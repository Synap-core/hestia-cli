...
import { loadConfig } from '../../../lib/utils/index.js';
...
export class OpenClawService {
    ...
    async installViaNpm(packageName, version) {
    ...
    async createDefaultConfig(installMethod, installPath) {
    ...
    async configure(config) {
        ...
        await this.saveOpenClawConfig(validated);
    ...
    async addSkill(name, code) {
        ...
        await fs.writeFile(path.join(skillDir, "meta.yaml"), YAML.stringify(metadata), "utf-8");
        // Write config if provided
        if (code.config) {
            await fs.writeFile(path.join(skillDir, "config.yaml"), YAML.stringify(code.config), "utf-8");
        }
        // Update config
    ...
    async getSkill(name) {
        ...
        try {
            ...
            let skillConfig;
            try {
                const configContent = await fs.readFile(path.join(skillDir, "config.yaml"), "utf-8");
    ...
    async updateSkill(name, updates) {
        ...
        if (updates.metadata) {
            ...
            await fs.writeFile(path.join(skillDir, "meta.yaml"), YAML.stringify(newMetadata), "utf-8");
        }
        // Update config if provided
        if (updates.config) {
            await fs.writeFile(path.join(skillDir, "config.yaml"), YAML.stringify(updates.config), "utf-8");
        }
        // Update config list
    ...
    async toggleSkill(name, enabled) {
        ...
        const skillDir = path.join(this.skillsDir, name);
        await fs.writeFile(path.join(skillDir, "meta.yaml"), YAML.stringify(config.skills[skillIndex]), "utf-8");
        // Hot reload if running
        if (config.hotReload && (await this.isRunning())) {
            await this.reloadSkills();
        }
     ...
     */
    async configureComms(platform, config) {
        const openClawConfig = await this.loadOpenClawConfig();
    ...
    async getCommsStatus() {
        ...
        for (const platform of ["telegram", "whatsapp", "discord", "imessage"]) {
     ...
     */
    async testCommsConnection(platform) {
        const config = await this.loadOpenClawConfig();
        const platformConfig = config.comms[platform];
    ...
    async getStatus() {
        ...
        if (isRunning) {
            try {
                ...
                apiStats = apiStatus.stats || apiStats;
    ...
    async loadOpenClawConfig() {
        try {
            ...
            return openClawConfigSchema.parse(parsed);
        }
        catch (error) {
            ...
            if (error instanceof z.ZodError) {
                throw new Error(`OpenClaw configuration is invalid:\n${error.errors
    ...
    setupProcessHandlers(process, config) {
    ...
    async waitForStartup(port, timeout) {
    ...
    async callOpenClawAPI(endpoint, options) {
        const config = await this.loadOpenClawConfig();
