 ...
 */
import { logger, section } from '../../lib/utils/index';
import { spinner } from '../../lib/utils/index';
import { serverProvisioner } from '../../../domains/provision/lib/server-provisioner.js';
import { hardwareMonitor } from '../../../domains/install/lib/hardware-monitor.js';
...
// MAIN COMMAND
// ============================================================================
export function provisionCommand(program) {
    const provision = program
        .command('provision')
        .description('Provision bare metal servers with Hestia')
    ...
    clusterCmd
        .command('detect')
        .description('Detect other Hestia nodes on network')
        .action(async () => {
    ...
    try {
        ...
        if (networkType === 'cgmat' || networkType === 'private') {
            ...
            if (enableTunnel) {
                ...
                logger.info(`  Run: ${chalk.bold('hestia tunnel:enable')} on this server`);
                logger.info(`  Run: ${chalk.bold('hestia tunnel:enable --mode server')} on a VPS`);
            }
        }
        ...
        logger.info('4. After installation, run: ' + chalk.cyan('hestia init'));
        logger.newline();
...
async function generateProfile(options) {
    ...
    if (save) {
        const profilePath = path.join(os.homedir(), '.hestia', 'profiles', `${profile.name}.json`);
...
async function configureCluster(nodes, options) {
    ...
    try {
        ...
        logger.info('1. Verify all nodes are running: ' + chalk.cyan('hestia status'));
        logger.info('2. Check cluster health: ' + chalk.cyan('hestia health cluster'));
    }
    catch (error) {
...
function generateMarkdownReport(report) {
    ...
    if (report.hardware.gpu.length > 0) {
        lines.push(`### GPUs`, ...report.hardware.gpu.map(g => `- ${g.model} (${g.vendor}): ${formatBytes(g.vram)} VRAM`), ``);
    }
    if (report.benchmarks) {
        lines.push('## Performance Benchmarks', '', `**Overall Score:** ${report.benchmarks.overall}/100`, '', `- CPU: ${report.benchmarks.cpu?.score.toFixed(1) || 0}/100`, `- Memory: ${report.benchmarks.memory?.score.toFixed(1) || 0}/100`, '');
    }
...
async function detectNetworkType() {
    try {
        ...
        const { stdout: publicIP } = await execAsync('curl -s https://api.ipify.org 2>/dev/null || echo ""', { timeout: 5000 });
        if (!publicIP.trim()) {
            return 'unknown';
        }
