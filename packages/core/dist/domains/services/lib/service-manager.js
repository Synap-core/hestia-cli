...
import { getOptionalService, getAllOptionalServices, defaultServiceConfigs, } from '../../../domains/services/lib/optional-services.js';
class ServiceManager {
    ...
    async saveStates() {
        ...
        await fs.writeFile(this.config.stateFilePath, JSON.stringify(data, null, 2), 'utf-8');
    }
    ...
    async findAvailablePort(serviceName, preferredPort) {
        ...
        for (let port = this.config.portRange.min; port <= this.config.portRange.max; port++) {
            if (this.config.reservedPorts.includes(port))
                continue;
    ...
    async install(serviceName) {
        ...
        await withSpinner(`Installing ${service.displayName}...`, async () => {
            // Check dependencies
    ...
    async remove(serviceName) {
        ...
        await withSpinner(`Removing ${service.displayName}...`, async () => {
            // Release ports
            const config = await this.loadServiceConfig(serviceName);
            if (config.ports) {
        ...
        for (const dep of service.dependencies) {
            if (dep.type === 'service') {
                ...
                if (!depService && !dep.optional) {
                    throw new Error(`Required dependency '${dep.name}' for ${service.displayName} is not available`);
                }
                if (depService) {
                    const depState = this.states.get(dep.name);
                    if (!depState?.installed && !dep.optional) {
                        throw new Error(`Required dependency '${depService.displayName}' for ${service.displayName} is not installed`);
                    }
                }
            }
    ...
    async start(serviceName) {
        ...
        await withSpinner(`Starting ${service.displayName}...`, async () => {
            await service.start();
            state.status = 'running';
            state.lastStartTime = new Date();
            await this.saveStates();
            // Wait a moment and verify it'actually running
    ...
    async stop(serviceName) {
        ...
        await withSpinner(`Stopping ${service.displayName}...`, async () => {
            await service.stop();
            state.status = 'installed';
            await this.saveStates();
    ...
    async configure(serviceName, config) {
        ...
        await withSpinner(`Configuring ${service.displayName}...`, async () => {
...
// Helper functions
export async function isServiceInstalled(serviceName) {
    const service = getOptionalService(serviceName);
    if (!service)
        return false;
    return await service.isInstalled();
}
export async function isServiceRunning(serviceName) {
    const service = getOptionalService(serviceName);
    if (!service)
        return false;
    const status = await service.status();
