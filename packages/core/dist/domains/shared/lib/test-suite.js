...
// Import all Hestia modules for testing
import { loadConfig, saveConfig, defaultConfig, createInitialConfig, validateConfig, mergeConfigs, loadCredentials, saveCredentials, } from '../../lib/utils/index';
...
import { OpenClaudeService, } from '../../../domains/ai/lib/openclaude-service.js';
import { OpenClawService, } from '../../../domains/ai/lib/openclaw-service.js';
import { logger, createLogger } from '../../lib/utils/index';
// ============================================================================
...
// ============================================================================
// Hestia Test Suite Class
// ============================================================================
export class HestiaTestSuite {
    config;
    testEnv = null;
    results = [];
    isRunning = false;
    ...
    async runAllTests() {
        ...
        this.log("header", "Hestia Test Suite");
    ...
    async runTests(category) {
        const startTime = new Date();
        this.results = [];
        this.log("header", `Hestia Test Suite - ${category.toUpperCase()}`);
        this.testEnv = await this.createTestEnvironment();
        const categoryTests = Array.from(this.testRegistry.entries()).filter(([, { category: cat }]) => cat === category);
        if (this.config.parallel) {
     ...
     */
    async watchMode(watchPath) {
        const targetPath = watchPath || process.cwd();
        this.log("header", "Hestia Test Suite - Watch Mode");
        this.log("info", `Watching: ${targetPath}`);
        // Run initial test
        await this.runAllTests();
        // Setup file watcher
        this.fileWatcher = watch(targetPath, { recursive: true, persistent: true }, async (eventType, filename) => {
    ...
    async testStateManager() {
        ...
        try {
            ...
            stateManager.setEnvVar("HESTIA_TEST_VAR", "test_value");
            const envValue = stateManager.getEnvVar("HESTIA_TEST_VAR");
    ...
    async testPackageService() {
        ...
        try {
            ...
            await fs.writeFile(path.join(packageDir, "package.yaml"), YAML.stringify(mockPackage), "utf-8");
            // Write mock docker-compose
            await fs.writeFile(path.join(packageDir, "docker-compose.yml"), `version: '3.8'
services:
  test:
    image: nginx:latest
`, "utf-8");
     ...
     */
    async testSynapBackendConnection() {
        const logs = [];
        const startTime = Date.now();
        try {
            // Check if backend is available
            const backendUrl = process.env.HESTIA_POD_URL || "http://localhost:4000";
     ...
     */
    async testOpenClaudeIntegration() {
        const logs = [];
        const startTime = Date.now();
        try {
...
`,
    ...
    async testPackageManagement() {
        ...
        try {
            ...
            await fs.mkdir(packageDir, { recursive: true });
            await fs.writeFile(path.join(packageDir, "package.yaml"), YAML.stringify(testPackage), "utf-8");
    ...
    async testCLI() {
        ...
        try {
            ...
            const commands = [
                { cmd: "hestia", args: ["--help"], description: "Show help" },
                { cmd: "hestia", args: ["--version"], description: "Show version" },
                { cmd: "hestia", args: ["status", "--help"], description: "Status command help" },
                { cmd: "hestia", args: ["config", "--help"], description: "Config command help" },
                { cmd: "hestia", args: ["package", "--help"], description: "Package command help" },
            ];
    ...
    async testPersistence() {
        ...
        try {
            ...
            await fs.writeFile(yamlFile, YAML.stringify(yamlData), "utf-8");
            ...
            await saveConfig({ ...defaultConfig, ...testConfig }, this.testEnv.configPath);
    ...
    async createTestEnvironment() {
        const tempDir = this.config.testEnvDir || await fs.mkdtemp(path.join(os.tmpdir(), "hestia-test-"));
        const env = {
            dir: tempDir,
            configPath: path.join(tempDir, ".hestia", "config.yaml"),
            packagesDir: path.join(tempDir, ".hestia", "packages"),
            tempFiles: [],
            processes: [],
            mocks: [],
            cleanup: async () => {
     ...
     * Retry a flaky test
     */
    async retry(fn, attempts = this.config.retries) {
        let lastError;
     ...
     */
    async timeout(fn, ms = this.config.timeout) {
        return Promise.race([
            fn(),
     ...
     */
    async executeTest(name, fn, category) {
        let retries = 0;
        let lastResult = null;
     ...
     */
    async execCommand(cmd, args, timeoutMs) {
        return new Promise((resolve, reject) => {
 ...
 */
export const testSuite = new HestiaTestSuite();
/**
    ...
    for (let i = 0; i < args.length; i++) {
        switch (args[i]) {
            ...
            case "--test":
                ...
                const singleSuite = new HestiaTestSuite(config);
...
Examples:
  hestia-test                          # Run all tests
  hestia-test --category unit          # Run unit tests only
  hestia-test --test testConfig        # Run single test
  hestia-test --ci                     # CI mode with JSON output
  hestia-test --watch                  # Watch mode
        `);
                process.exit(0);
                return;
        }
