...
export async function updateConfig(updates, customPath) {
...
export function getPackageConfig(config, packageName) {
    return config.packages[packageName];
}
// Set package configuration
export function setPackageConfig(config, packageName, packageConfig) {
    return {
        ...config,
        packages: {
            ...config.packages,
...
export function getIntelligenceConfig(config) {
    return config.intelligence;
}
// Set intelligence configuration
export function setIntelligenceConfig(config, intelligenceConfig) {
    return {
        ...config,
...
export async function createInitialConfig(options, customPath) {
    const config = {
        version: "1.0",
        hearth: {
            id: generateHearthId(),
...
export function getConfigSummary(config) {
    ...
    if (config.intelligence) {
        lines.push("", `Intelligence: ${config.intelligence.provider} (${config.intelligence.model})`);
    }
...
export async function loadCredentials(configDir) {
...
// Save credentials
export async function saveCredentials(credentials, configDir) {
    const paths = configDir
