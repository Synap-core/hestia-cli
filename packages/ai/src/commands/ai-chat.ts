#!/usr/bin/env node
/**
 * AI Chat Command - Manage AI chat UI interfaces for eve
 *
 * Provides commands for installing, configuring, and managing
 * optional AI chat UIs that connect to eve's AI backend.
 *
 * Commands:
 *   eve ai:chat              - List available chat UIs
 *   eve ai:chat:list         - Show installed and their status
 *   eve ai:chat:install      - Install specific UI
 *   eve ai:chat:remove       - Remove UI
 *   eve ai:chat:start        - Start UI
 *   eve ai:chat:stop         - Stop UI
 *   eve ai:chat:open         - Open browser to UI
 *   eve ai:chat:config       - Configure UI settings
 *   eve ai:chat:logs         - Show logs
 *
 * Providers:
...
import type { AIChatProvider } from "../lib/types/index";
...
async function displayStatus(): Promise<void> {
  ...
  if (statuses.length === 0) {
    ...
    console.log(chalk.gray("Run 'eve ai:chat' to see available options."));
    console.log(chalk.gray("Run 'eve ai:chat:install <provider>' to install one."));
    return;
  }
...
╚════════════════════════════════════════════════════╝
      ...
      console.log(chalk.gray("  eve ai:chat:list         - Show installed UIs and status"));
      console.log(chalk.gray("  eve ai:chat:install      - Install a chat UI"));
      console.log(chalk.gray("  eve ai:chat:remove       - Remove a chat UI"));
      console.log(chalk.gray("  eve ai:chat:start        - Start a chat UI"));
      console.log(chalk.gray("  eve ai:chat:stop         - Stop a chat UI"));
      console.log(chalk.gray("  eve ai:chat:open         - Open chat UI in browser"));
      console.log(chalk.gray("  eve ai:chat:config       - Configure a chat UI"));
      console.log(chalk.gray("  eve ai:chat:logs         - Show chat UI logs"));
    });
  ...
  aiChat
    ...
    .action(async (provider: string) => {
      ...
      try {
        ...
        console.log(chalk.gray(`Run 'eve ai:chat:open ${provider}' to open in browser`));
      } catch (error) {
    ...
    .action(async (provider: string) => {
      ...
      try {
        ...
        if (!status.running) {
          ...
          console.log(chalk.gray(`Run 'eve ai:chat:start ${provider}' first.`));
          process.exit(1);
        }
