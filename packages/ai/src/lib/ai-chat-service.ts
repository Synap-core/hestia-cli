...
import { loadConfig, updateConfig, getConfigPaths } from "./utils/index";
import type { AIChatProvider, AIChatConfig, AIChatProviderStatus } from "./types/index";
...
const PROVIDER_DEFINITIONS: Record<AIChatProvider, {
  name: string;
  description: string;
  defaultPort: number;
  image: string;
...
export class AIChatService {
  ...
  async install(provider: AIChatProvider): Promise<void> {
    ...
    const providerConfig: AIChatConfig["providers"][0] = {
      name: provider,
   ...
   */
  async getUrl(provider: AIChatProvider): Promise<string> {
    const definition = PROVIDER_DEFINITIONS[provider];
   ...
   */
  async remove(provider: AIChatProvider): Promise<void> {
    const definition = PROVIDER_DEFINITIONS[provider];
   ...
   */
  async logs(provider: AIChatProvider, follow = false): Promise<void> {
    const definition = PROVIDER_DEFINITIONS[provider];
   ...
   */
  async connectToAI(backend: "ollama" | "openclaude" | "openrouter"): Promise<void> {
   ...
   */
  async open(provider: AIChatProvider): Promise<void> {
   ...
   */
  private async checkComposeIncludesAIChat(composePath: string): Promise<boolean> {
    try {
   ...
   */
  private async appendAIChatToCompose(composePath: string): Promise<void> {
   ...
   */
  private async appendEnvVars(envPath: string, vars: Record<string, string>): Promise<void> {
    let content = "";
