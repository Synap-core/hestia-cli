/**
 * eve CLI - OpenClaude Service
 *
 * Wraps @gitlawb/openclaude CLI and integrates it with eve infrastructure.
...
import { logger, createLogger } from "./utils/index";
import { loadConfig, getConfigPaths, saveConfig, loadCredentials } from "./utils/index";
import type { eveConfig, IntelligenceConfig, Logger } from "./types/index";
...
export class OpenClaudeService {
  ...
  private eveConfig?: eveConfig;

  constructor(options: OpenClaudeServiceOptions = {}) {
    this.options = {
      configPath: options.configPath || path.join(os.homedir(), ".openclaude", "config.json"),
  ...
  /**
   * Start OpenClaude with eve configuration
   */
  ...
  async configureProvider(config: ProviderConfig): Promise<void> {
    ...
    try {
      // Update eve config
      const { config: eveConfig, path: configPath } = await loadConfig(this.options.configPath);
      
      eveConfig.intelligence = {
      ...
      await saveConfig(eveConfig, configPath);
  ...
  /**
   * Enable eve integration - exposes eve tools as MCP servers
   */
  async enableeveIntegration(options: {
    backendUrl?: string;
    apiKey?: string;
    workspaceId?: string;
  } = {}): Promise<void> {
    this.logger.info("Enabling eve integration...");
    ...
    try {
      ...
      const apiKey = options.apiKey || credentials.SYNAP_API_KEY || credentials.eve_API_KEY;
      const workspaceId = options.workspaceId || process.env.eve_WORKSPACE_ID;

      // Update profile with eve integration
      const profile = await this.loadProfile();
      profile.eveIntegration = {
        enabled: true,
        backendUrl,
        apiKey,
        workspaceId,
      ...
      // Install eve MCP server if not already installed
      if (!profile.mcpServers?.["eve"]) {
        profile.mcpServers = profile.mcpServers || {};
        profile.mcpServers["eve"] = {
          name: "eve",
          enabled: true,
          command: "npx",
      ...
      const envPath = path.join(this.options.workingDir, ".mcp-eve.env");
      const envContent = [
        `SYNAP_BACKEND_URL=${backendUrl}`,
        `SYNAP_API_KEY=${apiKey || ""}`,
        `eve_WORKSPACE_ID=${workspaceId || ""}`,
      ].join("\n");
      await fs.writeFile(envPath, envContent, "utf-8");

      this.logger.success("eve integration enabled");
      ...
      this.handleError("Failed to enable eve integration", error);
      throw error;
    }
  }
  ...
  async logActivity(type: string, details: Record<string, unknown>): Promise<void> {
    ...
    // This would integrate with eve's logging/audit system
  ...
  private async syncProfile(): Promise<void> {
    const profile = await this.loadProfile();
    const eveConfig = this.eveConfig || (await loadConfig(this.options.configPath)).config;

    // Map eve intelligence config to OpenClaude profile
    if (eveConfig.intelligence) {
      const intelligence = eveConfig.intelligence;
    ...
    profile.name = profile.name || "eve Profile";
  ...
  private mapProvider(provider: IntelligenceConfig["provider"]): string {
    // Map eve provider names to OpenClaude provider names
  ...
  private async buildEnvironment(): Promise<Record<string, string>> {
    ...
    // Add Synap Backend credentials for eve integration
    if (credentials.SYNAP_API_KEY) {
      env.SYNAP_API_KEY = credentials.SYNAP_API_KEY;
    }
    if (credentials.eve_API_KEY) {
      env.eve_API_KEY = credentials.eve_API_KEY;
    }

    // Add eve config
    if (config.connectors?.controlPlane?.url) {
      env.eve_POD_URL = config.connectors.controlPlane.url;
    }
