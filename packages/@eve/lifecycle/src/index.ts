...
function removePlanFor(comp: ComponentInfo): RemovePlan {
  ...
  switch (comp.id) {
    ...
    case "freellmapi":
      return { composeDir: "/opt/freellmapi", containerNames };
...
/**
 * FreeLLMAPI compose template — single source of truth.
 *
 * Mirrors the upstream docker-compose.yml
 * (https://github.com/tashfeenahmed/freellmapi) with two deliberate deviations:
 *  - `eve-brain-freellmapi` container name + the external `eve-network`, so the
 *    Synap IS can reach it by container name (`http://eve-brain-freellmapi:3001/v1`)
 *    rather than over the host.
 *  - NO published host port. Upstream publishes `127.0.0.1:3001` and warns the
 *    proxy is "single-user and guarded only by the unified API key". Inside Eve
 *    the dashboard is reached through Traefik at `llm.<domain>`, so binding a
 *    host port would only widen exposure for no gain.
 *
 * `ENCRYPTION_KEY` is REQUIRED in production — it encrypts the upstream provider
 * keys at rest in the SQLite volume. It is generated once at install and stored
 * in `.env` (0600). Regenerating it would orphan every stored key, which is why
 * the installer writes it only when absent.
 */
const FREELLMAPI_COMPOSE_YAML = `# FreeLLMAPI — generated by @eve/lifecycle (regenerated on every install/update)
services:
  freellmapi:
    image: ghcr.io/tashfeenahmed/freellmapi:\${FREELLMAPI_TAG:-latest}
    container_name: eve-brain-freellmapi
    restart: unless-stopped
    env_file:
      - .env
    environment:
    ...
    healthcheck:
      test: ["CMD", "node", "-e", "fetch('http://127.0.0.1:3001/api/ping').then(r=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))"]
      interval: 30s
      timeout: 5s
      start_period: 15s
      retries: 3
...
function writeFreellmapiCompose(deployDir: string): void {
  ...
  writeFileSync(join(deployDir, "docker-compose.yml"), FREELLMAPI_COMPOSE_YAML);
}

/**
 * Ensure `.env` carries an ENCRYPTION_KEY, generating one only if absent.
 *
 * Rotating this key silently orphans every provider key already encrypted in
 * the SQLite volume — the container comes up and every upstream call fails auth,
 * which reads like "the free tiers stopped working" rather than "Eve rotated
 * your key". So this is create-if-missing, never regenerate.
 */
function ensureFreellmapiEnv(deployDir: string): { created: boolean } {
  mkdirSync(deployDir, { recursive: true });
  ...
  if (/^ENCRYPTION_KEY=.+$/m.test(existing)) return { created: false };

  const key = randomBytes(32).toString("hex");
  const merged =
    (existing.trimEnd() ? existing.trimEnd() + "\n" : "") +
    `# Encrypts stored provider keys at rest. Rotating this orphans them.\nENCRYPTION_KEY=${key}\nPORT=3001\n`;
...
/**
 * Install FreeLLMAPI and register it as a LOW-PRIORITY pod provider.
 *
 * The key harvest is what makes this unattended. FreeLLMAPI mints its unified
 * key (`freellmapi-<48 hex>`) during its FIRST DB migration — before any
 * dashboard account exists — and writes it to stdout. So `docker compose logs`
 * yields it with no human clicking through a UI, and no account needs creating
 * at all. If the line is not found we do NOT fail the install: the container is
 * running and healthy, and the operator can read the key from the dashboard;
 * we just say so instead of pretending the provider was registered.
 *
 * PRIORITY 90 is deliberate. The IS cascade is ordered by ascending priority,
 * and `tags` is not read by its routing at all, so priority is the ONLY lever
 * that expresses "fallback lane". Registering this ahead of a paid provider
 * would silently move real work onto free tiers.
 *
 * COST 0 is also deliberate, and it is a correctness fix rather than cosmetics.
 * The IS cost meter prices a turn from the REQUESTED model's declared cost and
 * never reads the model the response reports — so a gateway that fails over
 * would otherwise bill free-tier tokens at some other model's rate. Declaring
 * the models at zero makes the money right; only which model gets the credit
 * stays approximate.
 */
async function* installFreellmapi(): AsyncGenerator<LifecycleEvent> {
  const deployDir = "/opt/freellmapi";

  writeFreellmapiCompose(deployDir);
  const { created } = ensureFreellmapiEnv(deployDir);
  yield {
    type: "log",
    line: created
      ? "Generated a new ENCRYPTION_KEY for FreeLLMAPI (stored 0600 in .env)"
      : "Reusing the existing FreeLLMAPI ENCRYPTION_KEY (rotating it would orphan stored provider keys)",
  };
  ...
  for (let i = 0; i < 30; i++) {
    try {
      const out = execSync(
        `docker exec eve-brain-freellmapi node -e "fetch('http://127.0.0.1:3001/livez').then(r=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))"`,
  ...
  if (!healthy) {
    yield { type: "log", line: "FreeLLMAPI did not report ready within 60s — skipping provider registration." };
    return;
  }
  ...
  if (!unifiedKey) {
    yield {
      type: "log",
      line:
        "FreeLLMAPI is running, but its unified API key was not found in the container logs " +
        "(it is printed only on FIRST boot). Read it from the dashboard, then run: " +
 ...
 *   `  Your unified API key: freellmapi-<48 hex>`
 * We match the KEY SHAPE rather than the sentence, so a reworded log line still
...
function readFreellmapiUnifiedKey(): string | null {
  try {
    return parseFreellmapiUnifiedKey(
      execSync(`docker logs eve-brain-freellmapi 2>&1`, {
        encoding: "utf-8",
        maxBuffer: 10 * 1024 * 1024,
        stdio: ["pipe", "pipe", "pipe"],
...
async function* registerFreellmapiProvider(unifiedKey: string): AsyncGenerator<LifecycleEvent> {
  ...
  if (!podUrl || !apiKey) {
    yield {
      type: "log",
      line: "No pod configured — FreeLLMAPI is running but not registered as a provider yet. Run `eve brain providers add freellmapi …` once the pod is up.",
    };
    return;
  }
  ...
  try {
    const result = await upsertPodProvider(podUrl, apiKey, {
      ...
      name: "FreeLLMAPI (free-tier gateway)",
      baseUrl: "http://eve-brain-freellmapi:3001/v1",
      apiKeyEnvVar: "FREELLMAPI_API_KEY",
      apiKey: unifiedKey,
      enabled: true,
      // Last in the cascade — see the docblock on installFreellmapi.
      priority: 90,
      tags: ["free-tier", "gateway", "fallback"],
      models: [
        {
          id: "auto",
          tier: "free",
          // The router picks a tool-capable model when the request carries
          // `tools`, and 4xxs upfront if none is enabled — so this is honest.
          supportsTools: true,
          costPer1MInput: 0,
          costPer1MOutput: 0,
        },
    ...
    yield { type: "log", line: describePodProviderResult(result) };
  } catch (err) {
    yield {
      type: "log",
      line: `FreeLLMAPI is running, but registering it on the pod failed: ${
