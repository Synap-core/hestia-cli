...
async function* installOpenWebUi(): AsyncGenerator<LifecycleEvent> {
  ...
  // Compose YAML notes — aligned with Open WebUI's official template
  // (https://github.com/open-webui/open-webui/blob/main/docker-compose.yaml)
  // with these deviations for our stack:
  ...
  //    anything pipelines wiring writes to `.env` (OPENAI_API_BASE_URLS,
  //    OPENAI_API_KEYS) would never reach the running container — chat
  //    requests would silently bypass the pipelines sidecar even
  //    though everything looked wired in the file. Listing the file
  //    explicitly makes every key in `.env` a container env var.
  //  - `extra_hosts: host.docker.internal:host-gateway` from the
  //    upstream template — lets OpenWebUI reach host services
  //    (Ollama on the host, etc.) without us hard-coding bridge IPs.
  //  - `WEBUI_SECRET_KEY=` empty default per upstream; the actual
  //    value is supplied via the .env file we generate alongside.
...
`;
  ...
  if (existsSync(owEnv)) {
    ...
    const synapKeyFromEnv = cur
      .split("\n")
      .find(l => l.startsWith("SYNAP_API_KEY="))?.split("=", 2)[1] ?? synapApiKey;
