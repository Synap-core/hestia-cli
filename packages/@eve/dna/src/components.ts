...
export interface ServiceInfo {
   ...
   * `pathProxies` pins specific paths to a different port on the same container
   * (higher priority router). Example: Nango's Connect UI SPA runs on 3009 but
   * `/env.js` must be served by the API on 3003 because it's dynamically generated.
   */
  additionalSubdomains?: Array<{
    subdomain: string;
    internalPort: number;
    pathProxies?: Array<{ path: string; internalPort: number }>;
  }>;
}
