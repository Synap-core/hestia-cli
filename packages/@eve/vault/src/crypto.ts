...
export async function encryptWithKey(
  ...
  const ct = await crypto.subtle.encrypt({ name: ALGORITHM, iv }, key, new TextEncoder().encode(data));
...
export async function tryUnlock(
  password: string,
  meta: { salt: string; verificationCipher: string; verificationIv: string; verificationTag: string },
): Promise<CryptoKey | null> {
  try {
    const key = await deriveVaultKey(password, meta.salt);
    const pt = await decryptWithKey(meta.verificationCipher, meta.verificationIv, meta.verificationTag, key);
