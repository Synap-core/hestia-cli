...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    effectiveRouting,
    telegram: {
      enabled: channels.telegram?.enabled ?? hasToken(channels.telegram?.botToken),
    ...
    discord: {
      enabled: channels.discord?.enabled ?? hasToken(channels.discord?.botToken),
    ...
    whatsapp: {
      enabled: channels.whatsapp?.enabled ?? hasToken(channels.whatsapp?.accessToken),
    ...
    signal: {
      enabled: channels.signal?.enabled ?? !!(channels.signal?.phoneNumber?.trim()),
      phoneNumber: channels.signal?.phoneNumber ?? null,
      apiUrl: channels.signal?.apiUrl ?? null,
    },
    matrix: {
      enabled: channels.matrix?.enabled ?? hasToken(channels.matrix?.accessToken),
    ...
    slack: {
      enabled: channels.slack?.enabled ?? hasToken(channels.slack?.botToken),
