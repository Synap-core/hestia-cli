  ...
  if (hasLibreChat) {
    steps.push({
      ...
      async fn() {
        ...
        // Resolve bundled template paths
        const __filename = fileURLToPath(import.meta.url);
        const templateBase = pathJoin(__filename, '../../../../../packages/install/src/templates');

        // Write librechat.yaml (Synap endpoint + MCP config)
        const yamlSrc = pathJoin(templateBase, 'librechat.yaml');
        if (existsSync(yamlSrc)) {
          writeFileSync(pathJoin(deployDir, 'librechat.yaml'), readFileSync(yamlSrc));
        }

        // Read Synap secrets for the .env
        ...
        const gen = (n = 32) => randomBytes(n).toString('hex');
