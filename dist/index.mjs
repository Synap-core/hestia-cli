...
// packages/@eve/brain/src/lib/exec.ts
import { spawn } from "child_process";
function execa(command, args, options) {
  return new Promise((resolve, reject) => {
    const child = spawn(command, args, {
      stdio: (options == null ? void 0 : options.stdio) === "inherit" ? "inherit" : "pipe",
...
// packages/@eve/brain/src/lib/synap-delegate.ts
import { existsSync } from "fs";
import { join } from "path";
function resolveSynapDelegate() {
  var _a, _b;
  const repoRoot = (_a = process.env.SYNAP_REPO_ROOT) == null ? void 0 : _a.trim();
  if (!repoRoot || !existsSync(repoRoot)) {
    return null;
  }
  const script = ((_b = process.env.SYNAP_CLI) == null ? void 0 : _b.trim()) || join(repoRoot, "synap");
...
// packages/@eve/brain/src/lib/synap.ts
...
// packages/@eve/dna/dist/index.js
...
var OrganSchema = external_exports.enum(["brain", "arms", "builder", "eyes", "legs"]);
var OrganConfigSchema = external_exports.object({
...
var ConfigSchema = external_exports.object({
  ...
  aiModel: external_exports.enum(["ollama", "none"]).default("none"),
  organs: external_exports.record(OrganConfigSchema),
  settings: external_exports.object({
    ...
    defaultTimeout: external_exports.number().default(3e4)
  }),
  createdAt: external_exports.string().datetime().or(external_exports.date()),
  updatedAt: external_exports.string().datetime().or(external_exports.date())
});
var defaultConfig = {
  version: "0.1.0",
  aiModel: "none",
...
function ensureAllOrgans(organs) {
  ...
  for (const organ of allOrgans) {
    result[organ] = {
      enabled: ((_a = organs[organ]) == null ? void 0 : _a.enabled) ?? false,
      autoStart: ((_b = organs[organ]) == null ? void 0 : _b.autoStart) ?? false,
      port: (_c = organs[organ]) == null ? void 0 : _c.port,
      environment: (_d = organs[organ]) == null ? void 0 : _d.environment
    };
  }
  return result;
}
var ConfigManager = class {
   ...
   */
  getConfigDir() {
    return join2(homedir(), ".config", "eve");
  }
  /**
   * Get the full path to the configuration file
   */
  getConfigPath() {
    return join2(this.getConfigDir(), "config.yaml");
  }
  /**
  ...
  async saveConfig(config) {
    try {
      ...
      const content = jsYaml.dump(configFile, {
        indent: 2,
        lineWidth: -1,
        noRefs: true,
        sortKeys: false
...
var CredentialsManager = class {
  ...
  getCredentialsPath() {
    ...
    return join22(homedir2(), ".config", "eve", "credentials");
  }
  /**
  ...
  async ensureCredentialsDir() {
    const credsDir = join22(homedir2(), ".config", "eve");
    try {
...
var StateSchema = external_exports.object({
  ...
  initializedAt: external_exports.string(),
  aiModel: external_exports.enum(["ollama", "none"]),
  organs: external_exports.record(
    external_exports.enum(["brain", "arms", "builder", "eyes", "legs"]),
    external_exports.object({
      state: external_exports.enum(["missing", "installing", "starting", "ready", "error", "stopped"]),
  ...
  metadata: external_exports.object({
    lastBootTime: external_exports.string().optional(),
...
var EntityStateManager = class {
  ...
  async saveState(state) {
    ...
    try {
      ...
      const json2 = JSON.stringify(state, null, 2);
      await writeFile(statePath, json2, "utf-8");
      this.state = state;
    } catch (error) {
      if (error instanceof external_exports.ZodError) {
  ...
  async updateOrgan(organ, organState, options) {
    ...
    if (options == null ? void 0 : options.version) {
      status.version = options.version;
    }
    if ((options == null ? void 0 : options.errorMessage) && organState === "error") {
  ...
  async getReadyOrgans() {
    ...
    return ORGANS.filter((organ) => {
      var _a;
      return ((_a = state.organs[organ]) == null ? void 0 : _a.state) === "ready";
    });
  }
  async getMissingOrgans() {
    ...
    return ORGANS.filter((organ) => {
      var _a;
      return ((_a = state.organs[organ]) == null ? void 0 : _a.state) === "missing";
    });
  }
  async getErrorOrgans() {
    ...
    return ORGANS.filter((organ) => {
      var _a;
      return ((_a = state.organs[organ]) == null ? void 0 : _a.state) === "error";
    });
  }
  calculateCompleteness(state) {
    const readyCount = ORGANS.filter((organ) => {
      var _a;
      return ((_a = state.organs[organ]) == null ? void 0 : _a.state) === "ready";
    }).length;
...
var SetupProfileKindSchema = external_exports.enum(["inference_only", "data_pod", "full"]);
var TunnelProviderSchema = external_exports.enum(["pangolin", "cloudflare"]);
var BuilderEngineSchema = external_exports.enum(["opencode", "openclaude", "claudecode"]);
var AiModeSchema = external_exports.enum(["local", "provider", "hybrid"]);
var AiProviderSchema = external_exports.enum(["ollama", "openrouter", "anthropic", "openai"]);
var SetupProfileSchema = external_exports.object({
  version: external_exports.literal("1"),
  profile: SetupProfileKindSchema,
  updatedAt: external_exports.string(),
  domainHint: external_exports.string().optional(),
  hearthName: external_exports.string().optional(),
  source: external_exports.enum(["wizard", "usb_manifest", "cli"]).optional(),
  ...
  tunnelDomain: external_exports.string().optional(),
  ...
  network: external_exports.object({
    exposureMode: external_exports.enum(["local", "public"]),
    synapHost: external_exports.string(),
    legs: external_exports.object({
      tunnelProvider: TunnelProviderSchema.optional(),
      hostStrategy: external_exports.enum(["same_as_synap", "custom"]).optional(),
      host: external_exports.string().optional()
    }).optional()
  }).optional(),
  /** Non-secret Synap install preferences used to resume setup after interruption. */
  synapInstall: external_exports.object({
    mode: external_exports.enum(["auto", "from_image", "from_source"]).optional(),
    tlsEmail: external_exports.string().optional(),
    withOpenclaw: external_exports.boolean().optional(),
    withRsshub: external_exports.boolean().optional(),
    adminBootstrapMode: external_exports.enum(["token", "preseed"]).optional(),
...
var UsbSetupManifestSchema = external_exports.object({
  version: external_exports.literal("1"),
  target_profile: SetupProfileKindSchema,
  hearth_name: external_exports.string().optional(),
  domain_hint: external_exports.string().optional(),
  tunnel_provider: TunnelProviderSchema.optional(),
  tunnel_domain: external_exports.string().optional()
});
var execFileAsync = promisify(execFile);
var AiProviderSchema2 = external_exports.enum(["ollama", "openrouter", "anthropic", "openai"]);
var AiModeSchema2 = external_exports.enum(["local", "provider", "hybrid"]);
var SecretsSchema = external_exports.object({
  ...
  builder: external_exports.object({
    ...
    codeEngine: external_exports.enum(["opencode", "openclaude", "claudecode"]).optional(),
  ...
  arms: external_exports.object({
    ...
    messaging: external_exports.object({
      enabled: external_exports.boolean().optional(),
      platform: external_exports.enum(["telegram", "signal", "matrix"]).optional(),
...
// packages/@eve/brain/src/lib/ollama.ts
var OllamaService = class {
   ...
   */
  async start(options) {
    const publishToHost = (options == null ? void 0 : options.publishToHost) !== false;
...
// packages/@eve/brain/src/commands/init.ts
...
async function runBrainInit(options) {
  ...
  const repo = ((_a = options.synapRepo) == null ? void 0 : _a.trim()) || ((_b = process.env.SYNAP_REPO_ROOT) == null ? void 0 : _b.trim()) || void 0;
  if (repo) {
  ...
  const domain = ((_c = options.domain) == null ? void 0 : _c.trim()) || "localhost";
  const email = ((_d = options.email) == null ? void 0 : _d.trim()) || ((_e = process.env.LETSENCRYPT_EMAIL) == null ? void 0 : _e.trim()) || ((_f = process.env.SYNAP_LETSENCRYPT_EMAIL) == null ? void 0 : _f.trim());
  const adminEmail = ((_g = options.adminEmail) == null ? void 0 : _g.trim()) || ((_h = process.env.ADMIN_EMAIL) == null ? void 0 : _h.trim());
  const adminPassword = ((_i = options.adminPassword) == null ? void 0 : _i.trim()) || ((_j = process.env.ADMIN_PASSWORD) == null ? void 0 : _j.trim());
...
// packages/@eve/brain/src/commands/status.ts
...
// packages/@eve/brain/src/lib/postgres.ts
...
// packages/@eve/brain/src/lib/redis.ts
...
// packages/@eve/legs/dist/index.js
...
var InferenceGateway = class {
  ...
  constructor(cwd = process.cwd(), hostPort = DEFAULT_HOST_PORT) {
    var _a;
    this.cwd = cwd;
    this.baseDir = join42(cwd, ".eve", "inference-gateway");
    this.hostPort = ((_a = process.env.EVE_INFERENCE_GATEWAY_PORT) == null ? void 0 : _a.trim()) || hostPort;
  }
  ...
  htpasswdLine(username, plainPassword) {
    ...
    if (r.error || r.status !== 0) {
      throw new Error(`openssl passwd -apr1 failed: ${r.stderr || ((_a = r.error) == null ? void 0 : _a.message) || "unknown"}`);
    }
  ...
  async ensure(ollamaHost = "http://eve-brain-ollama:11434") {
    var _a;
    mkdirSync4(join42(this.baseDir, "dynamic"), { recursive: true });
    const username = "eve";
...
log:
  level: INFO
`.trim();
    writeFileSync4(join42(this.baseDir, "dynamic", "ollama-users"), `${userLine}
`, { mode: 384 });
    const dynamicYaml = [
      "http:",
      "  routers:",
    ...
    writeFileSync4(join42(this.baseDir, "traefik.yml"), staticYaml);
    writeFileSync4(join42(this.baseDir, "dynamic", "ollama.yml"), dynamicYaml);
    const secretsFile = join42(this.baseDir, "..", "secrets", "ollama-gateway.txt");
    mkdirSync4(join42(this.baseDir, "..", "secrets"), { recursive: true });
...
`;
    ...
    await writeEveSecrets(
      {
        inference: {
          ...(prevSecrets == null ? void 0 : prevSecrets.inference) ?? {},
          gatewayUrl: `http://127.0.0.1:${this.hostPort}`,
          gatewayUser: username,
          gatewayPass: password,
          ollamaUrl: ((_a = prevSecrets == null ? void 0 : prevSecrets.inference) == null ? void 0 : _a.ollamaUrl) ?? ollamaHost
        }
      },
      this.cwd
    );
...
// packages/@eve/brain/src/inference-init.ts
...
// packages/@eve/brain/src/index.ts
