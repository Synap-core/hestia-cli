...
```
packages/
├── hestia-cli/            # Orchestrator (thin)
├── @hestia/brain/         # Brain organ
├── @hestia/arms/          # Arms organ
├── @hestia/legs/          # Legs organ
├── @hestia/builder/       # Builder organ
├── @hestia/eyes/          # Eyes organ
├── @hestia/heart/         # Heart organ
├── @hestia/dna/           # DNA (config)
└── @hestia/usb/           # Birth (USB)
```
...
**New Thin CLI:**
```typescript
// packages/hestia-cli/src/index.ts
import { Command } from 'commander'
import { initBrain } from '@hestia/brain'
import { aiCommand } from '@hestia/arms'
import { tunnelCommand } from '@hestia/legs'
// ... etc
...
**New Command:** `hestia status` (entity view)
  ...
  async predictIssues(command: string): Promise<Warning[]> {
    ...
    if (command.includes('arms') && state.organs.brain === 'missing') {
      warnings.push({
        ...
        suggestion: 'Run "hestia brain init" first'
      })
    }
...
**File:** `packages/hestia-cli/src/legacy-adapter.ts`
...
export const LEGACY_COMMANDS: Record<string, string> = {
  ...
  'hestia tunnel': 'hestia legs tunnel',
  'hestia usb': 'hestia birth create-usb',
}
...
npm install -g @hestia/brain @hestia/arms @hestia/legs @hestia/builder

# Migrate config
hestia dna migrate --from=legacy --to=organs

# Verify migration
hestia doctor

echo "✅ Migration complete!"
echo "Run 'hestia status' to see your entity."
```
...
export function legacyCommand(program: Command): void {
  program
    ...
    .action(async () => {
      ...
      console.warn('   Use: hestia organ command')
      console.warn('   See: https://docs.hestia.sh/migration')
...
- [ ] `hestia status` shows entity view
- [ ] `hestia doctor` diagnoses issues
- [ ] `hestia grow` suggests improvements
...
This refactor transforms Hestia from a **CLI tool** into an **Entity Creation System**.
